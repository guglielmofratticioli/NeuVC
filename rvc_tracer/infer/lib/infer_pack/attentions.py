...
class MultiHeadAttention(nn.Module):
    ...
    def attention(
        ...
        def _get_relative_embeddings_if_tracing(relative_embeddings, length, window_size):
            max_relative_position = 2 * window_size + 1
            # Pad first before slice to avoid using cond ops.
            pad_length = torch.max(torch.stack((length - (window_size + 1), torch.tensor(0))))
            slice_start_position = torch.max(torch.stack(((window_size + 1) - length, torch.tensor(0))))
            slice_end_position = slice_start_position + 2 * length - 1
            if pad_length > 0:
                padded_relative_embeddings = F.pad(
                    relative_embeddings,
                    # commons.convert_pad_shape([[0, 0], [pad_length, pad_length], [0, 0]]),
                    [0, 0, int(pad_length.item()), int(pad_length.item()), 0, 0],
                )
        ...
        if self.window_size is not None:
            ...
            if torch.jit.is_tracing():
                key_relative_embeddings = _get_relative_embeddings_if_tracing(self.emb_rel_k, t_s, torch.tensor(self.window_size))
            else:
        ...
        if self.window_size is not None:
            ...
            if torch.jit.is_tracing():
                value_relative_embeddings = _get_relative_embeddings_if_tracing(self.emb_rel_v, t_s, torch.tensor(self.window_size))
            else:
