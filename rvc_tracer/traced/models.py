...
class SynthesizerTrnMs256NSFsid(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if kwargs["version"]:
            ...
            self.version = None
        if kwargs["if_f0"] is not None:
            self.if_f0 = kwargs["if_f0"]
        else:
            self.if_f0 = None
    ...
    def forward(
        ...
        def _get_z_p(m_p, logs_p, x_mask):
            z_p = (m_p + torch.exp(logs_p) * torch.rand_like(m_p) * 0.66666) * x_mask
            return z_p
        z_p = _get_z_p(m_p, logs_p, x_mask)
    ...
    def get_synth_params(self):
        if self.version and self.if_f0 is not None:
            return self.version, self.if_f0, self.sr
...
class SynthesizerTrnMs256NSFsid_nono(nn.Module):
    ...
    def get_synth_params(self):
        ...
        #     args.append(value)
        # return *args , self.sr
        # if self.kwargs["if_fo"] and self.kwargs["version"] is not None:
        #     return self.kwargs["if_fo"], self.kwargs["version"], self.sr
        if self.version and self.if_f0 is not None:
            return self.version, self.if_f0, self.sr
